...
## 벡터 검색과 의미 기반 검색
Elasticsearch는 벡터 검색도 지원한다.  
기존 키워드 검색은 단어 일치를 중심으로 검색한다.
...
벡터 검색은 의미가 비슷한 데이터를 찾는다.

```
검색어: 여름에 입기 좋은 상의
결과: 반팔 티셔츠, 린넨 셔츠, 얇은 니트
```

Elastic 공식 문서에서는 Elasticsearch가 자연어 처리와 벡터 검색을 사용해 semantic search 기능을 제공한다고 설명한다.  
이 기능은 AI 검색, 추천, RAG에서 중요하다.
