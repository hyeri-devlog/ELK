...
### 1) 고성능 전문 검색
Lucene의 가장 중요한 특징은 **Full-text Search**, 즉 전문 검색이다.  
전문 검색은 긴 텍스트 안에서 원하는 단어를 찾아내는 검색이다.  

예를 들어 게시글 본문이 다음과 같다고 가정해보자.

```
SpringBoot와 Elasticsearch를 연동해서 상품 검색 기능을 구현했다.
```

사용자가 다음 단어로 검색해도 관련 문서를 찾아야 한다.
...
Lucene은 이런 검색을 빠르게 처리하기 위해 텍스트를 분석하고, 토큰으로 나누고, 역색인을 생성한다.

### 2) 색인 기능
Lucene은 데이터를 그냥 저장하지 않는다.  
검색하기 좋은 구조로 바꿔서 저장한다.  
이 과정을 **색인, Indexing**이라고 한다.  

예를 들어 다음 문장이 있다고 하겠다.

```
검정색 반팔 티셔츠
```

검색 엔진은 이 문장을 내부적으로 검색 가능한 단위로 나눈다.
...
이 과정을 통해 나중에 사용자가 `반팔`이라고 검색했을 때 빠르게 결과를 찾아낼 수 있다.

### 3) 분석과 토큰화
Lucene은 텍스트를 검색 가능한 단위로 나누는 기능을 제공한다.  
이때 중요한 개념이 다음 두 가지이다.

|개념|설명|
|:--:|:--|
|Analyzer|텍스트를 어떻게 분석할지 정하는 전체 처리기|
|Tokenizer|문장을 단어 단위로 쪼개는 처리기|

예를 들어 영어 문장에서는 공백 기준으로 나누는 것이 비교적 쉽다.
...
이런 표현들을 잘 검색하려면 한국어 형태소 분석기나 n-gram 같은 분석 방식이 필요하다.  
즉, Elasticsearch에서 검색 품질을 높이려면 단순히 데이터를 넣는 것만이 아니라 "어떻게 분석해서 색인할 것인가"가 중요하다.

### 4) 역색인 구조
Lucene의 핵심은 역색인이다.  

일반적인 데이터 구조가 아래와 같다면,
...
그래서 검색어가 들어오면 전체 문서를 하나씩 뒤지지 않고, 해당 단어가 들어 있는 문서 목록을 바로 찾는다.  
이것이 Elasticsearch 검색 속도의 핵심이다.

### 5) 관련도 점수 계산
Elasticsearch에서 검색 결과는 단순히 포함됨/포함 안 됨으로만 나오지 않는다.  
검색어와 얼마나 관련 있는지 점수를 계산해서 더 관련성 높은 문서를 위로 올린다.  

예를 들어 사용자가 다음과 같이 검색했다고 해보겠다.

```
검정 반팔 티셔츠
```

검색 결과가 다음과 같이 있을 수 있다.
...
이때 `검정 반팔 티셔츠`라는 상품이 가장 위에 나와야 자연스럽다.  

Lucene은 이런 관련도 점수를 계산하는 기능을 제공한다.  
Elasticsearch는 이 기능을 활용해서 검색 결과를 정렬한다.

### 6) 하이라이팅
Lucene은 검색 결과에서 사용자가 검색한 단어를 강조하는 기능도 제공한다.  
예를 들어 사용자가 `반팔`을 검색하면 결과를 이렇게 보여줄 수 있다.

```
검정색 <em>반팔</em> 티셔츠
```

게시판이나 쇼핑몰 검색 결과에서 검색어가 굵게 표시되는 기능이 여기에 해당한다.

### 7) 맞춤법 교정과 추천어
Lucene은 맞춤법 교정, 추천어 기능도 지원한다.  

예를 들어 사용자가 오타를 입력했다고 해보겠다.
...
검색 시스템은 이런 입력을 보고 비슷한 단어를 추천할 수 있다.

```
혹시 elasticsearch를 찾으셨나요?
```

이런 기능은 검색 품질을 높이는 데 중요하다.

### 8) 벡터 검색 지원
최근 검색 시스템에서는 키워드 검색뿐 아니라 벡터 검색도 중요해졌다.  
벡터 검색은 단어가 정확히 일치하지 않아도 "의미가 비슷한 문서"를 찾는 방식이다.  

예를 들어 사용자가 이렇게 검색했다고 해보자.

```
비 오는 날 신기 좋은 신발
```

하지만 의미 기반 검색에서는 다음과 같은 상품도 찾을 수 있다.
...
Lucene 공식 문서에서도 고차원 벡터에 대한 nearest-neighbor search를 지원한다고 설명한다.

---

## 11. Elasticsearch 이해하기
Elasticsearch를 처음 배울 때는 다음 관점으로 보면 이해가 쉽다.

- **Elasticsearch**: 검색용 데이터베이스처럼 보이는 분산 검색 엔진
- **Lucene**: Elasticsearch 내부에서 실제 검색을 담당하는 핵심 검색 라이브러리
- **역색인**: 빠른 검색을 가능하게 하는 핵심 자료구조
- **Analyzer**: 텍스트를 검색 가능한 단어로 쪼개는 처리기
- **Query DSL**: Elasticsearch에 검색 요청을 보내는 JSON 문법

그리고 실무에서는 보통 아래 구조로 사용한다.
...
상품이나 게시글의 원본 데이터는 RDB에 저장하고,  
검색에 필요한 데이터는 Elasticsearch에 복사해서 저장한다.  

그래서 최종 구조는 보통 이렇게 된다.
...
SpringBoot
 ↓
RDB                 Elasticsearch
원본 데이터 저장       검색용 데이터 저장
```

---

## 12. 정리
`Elasticsearch`는 **대량의 데이터를 빠르게 검색하고 분석하기 위한 분산 검색 엔진**이다.  

`RDB`가 정확한 **데이터 저장, 관계 관리, 트랜잭션 처리**에 강하다면,  
`Elasticsearch`는 **전문 검색, 로그 분석, 빠른 집계, 검색 결과 관련도 계산**에 강하다.  

`Elasticsearch`는 내부적으로 `Apache Lucene`을 사용한다.  
`Lucene`은 **Java 기반의 고성능 검색 라이브러리**이며, **색인, 전문 검색, 분석, 토큰화, 하이라이팅, 맞춤법 교정, 벡터 검색** 같은 기능을 제공한다.  
`Elasticsearch`는 `Lucene` 위에 **분산 처리, REST API, JSON 문서 저장, 클러스터 관리, 샤딩, 복제, Kibana 연동** 같은 기능을 얹은 검색 엔진이다.  

따라서 Elasticsearch를 배운다는 것은 단순히 새로운 DB 하나를 배우는 것이 아니라, 아래를 배우는 것에 가깝다.

- 검색을 빠르게 하는 방법
- 텍스트를 검색 가능한 구조로 바꾸는 방법
- 대량 로그를 분석하는 방법
- AI 검색과 벡터 검색의 기반을 이해하는 것
...
[Lucene 공식 문서] Apache Lucene 핵심 기능 설명
